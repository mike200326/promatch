...
useEffect(() => {
  const fetchRecommendation = async () => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:8080/api/predict/clusters", {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
            ...
            {recommendedCluster && (
              <Card className="p-6 bg-white shadow-lg rounded-2xl">
                ...
                <CardContent>
                  ...
                  <button
                    onClick={() => {
                      const match = recommendedCluster.recommendation.match(/Mejorar en: (.+)/);
                      const examTitle = match ? match[1].split(",")[0].trim() : "Examen";
                      router.push(`/examen?title=${encodeURIComponent(examTitle)}`);
                    }}
